...
public class ForgetPasswordController {
    ...
    public Map cheKTOkenValid(@RequestBody ResetPasswordModel model) {
        if (model.getOtp() == null) return templateCRUD.templateError("Token " + config.isRequired);
    ...
    public Map<String, String> resetPassword(@RequestBody ResetPasswordModel model) {
        if (model.getOtp() == null) return templateCRUD.templateError("Token " + config.isRequired);
        if (model.getNewPassword() == null) return templateCRUD.templateError("New Password " + config.isRequired);
        User user = userRepository.findOneByOTP(model.getOtp());
        String success;
        if (user == null) return templateCRUD.templateError("Token not valid");
